...
const SingleBeer = ({ tagline, name, image_url, id }) => {
  return (
    <li className='max-w-sm  text-slate-100 shadow-xl p-4 bg-beer rounded-lg hover:scale-105 duration-300'>
      <Link
        to={`/details/${id}`}
        className='flex flex-col justify-center  rounded-lg  md:flex-row md:max-w-md bg-gray-200 '>
        {image_url !== null ? (
          <img
            src={image_url}
            alt={name}
